...
src_dir="/stor/scratch/Ochman/kristen/pangenome/redownloaded_CDSs"
dest_dir="/stor/scratch/Ochman/kristen/bacterial_species_CDSs"
...
while IFS=$'\t' read -r accession species; do
    # Find the corresponding fasta file using the accession number
    ...
    if [[ -n "$file" ]]; then
        ...
        new_file="${species}_$(printf "%02d" ${species_count["$species"]})_cds.fna"
