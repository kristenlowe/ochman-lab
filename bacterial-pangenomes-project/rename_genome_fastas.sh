# Define the source and destination directories for genome files
src_dir="/stor/scratch/Ochman/kristen/pangenome/pangenome_project_genomes"
dest_dir="/stor/scratch/Ochman/kristen/bacterial_species_genomes"
...
while IFS=$'\t' read -r accession species; do
    # Find the corresponding genome file using the accession number
    file=$(ls ${src_dir}/${accession}_*.fna 2>/dev/null)
    ...
    if [[ -n "$file" ]]; then
        ...
        new_file="${species}_$(printf "%02d" ${species_count["$species"]})_genome.fna"
